...
public abstract class BaseUpgradePortletPreferences extends UpgradeProcess {
	...
	protected void deletePortletPreferencesByOwnerType(
			int ownerType, String whereClause, String[]... joinTables)
		throws Exception {

		StringBundler sb = new StringBundler(7 * joinTables.length + 5);
	...
	protected void updatePortletPreferences() throws Exception {
		...
		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_ARCHIVED, whereClause,
			new String[] {"portletItemId", "PortletItem"});

		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_COMPANY, whereClause,
			new String[] {"companyId", "Company"});

		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_GROUP, whereClause,
			new String[] {"groupId", "Group_"});

		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_LAYOUT, whereClause,
			new String[] {"plid", "Layout"},
			new String[] {"layoutRevisionId", "LayoutRevision"});

		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_ORGANIZATION, whereClause,
			new String[] {"organizationId", "Organization"});

		deletePortletPreferencesByOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_USER, whereClause,
			new String[] {"userId", "User_"});

		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_ARCHIVED, whereClause, "PortletItem",
			"portletItemId");

		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_COMPANY, whereClause, "Company",
			"companyId");

		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_GROUP, whereClause, "Group_",
			"groupId");

		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_LAYOUT, whereClause, "Layout", "plid");

		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_LAYOUT, whereClause, "LayoutRevision",
			"layoutRevisionId");
		...
		updatePortletPreferencesWithOwnerType(
			PortletKeys.PREFS_OWNER_TYPE_USER, whereClause, "User_", "userId");
	}
	...
	protected void updatePortletPreferencesWithOwnerType(
			int ownerType, String whereClause, String rightTableName,
		...
		String sql = StringBundler.concat(
			...
			rightTableName, " on PortletPreferences.ownerId = ", rightTableName,
			StringPool.PERIOD, joinColumnName, whereClause);
